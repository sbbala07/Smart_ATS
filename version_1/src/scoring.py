...
def compute_semantic_similarity(resume_text, jd_text): # Resume content, JD content and gives simarlity content
    embeddings = model.encode([resume_text, jd_text])  # Generate embeddings with a list of two texts
    similarity = cosine_similarity([embeddings[0]], [embeddings[1]])[0][0]  # Cosine similarity b/w resume embedding, JD embedding
